		...
		<table>
			...
			<c:forEach items="${cart.cartItems}" var="cartItem" varStatus="status">
				<tr>
					...
					<td id="itemPrice${status.index}"><fmt:formatNumber value="${f:h(cartItem.goods.price)}" type="CURRENCY"
						currencySymbol="&yen;" maxFractionDigits="0" /></td>
					<td id="itemQuantity${status.index}">${f:h(cartItem.quantity)}</td>
					<td><input type="checkbox" name="removedItemsIds" id="removedItems${status.index}" value="${f:h(cartItem.goods.id)}" /></td>
				</tr>
			</c:forEach>
			<tr>
				<td>Total</td>
				<td id="totalPrice"><fmt:formatNumber value="${f:h(cart.totalAmount)}" type="CURRENCY" 
					currencySymbol="&yen;"  maxFractionDigits="0" /></td>
					<td></td>
					<td></td>
			</tr>
