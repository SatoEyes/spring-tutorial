...
<div>
	...
	<table>
		<tr>
			...
			<td id="orderNumer">${f:h(order.id)}</td>
		</tr>
		<tr>
	...
	<table>
		...
		<c:forEach items="${order.orderLines}" var="orderLine" varStatus="status">
			<tr>
				...
				<td id="itemPrice${status.index}"><fmt:formatNumber	value="${orderLine.goods.price}" type="CURRENCY"
		...
		<tr>
			<td>Total</td>
			<td id="totalPrice"><fmt:formatNumber value="${f:h(order.totalAmount)}" type="CURRENCY"
			 currencySymbol="&yen;" maxFractionDigits="0" /></td>
			<td></td>
		</tr>
	</table>
